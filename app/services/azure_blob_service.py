...
class AzureBlobService:
    def __init__(self):
        ...
        # Chave usada como Bearer ("App <key>") pra baixar mídia hospedada pelo Infobip.
    ...
    def upload_from_url(self, media_url, container_name, blob_name):
        """
        Baixa a mídia da URL do Infobip e sobe pro Azure Blob Storage.

        ⚠ REVISAR MANUALMENTE: dependendo da config do tenant Infobip, as URLs
        de mídia podem ser públicas (sem auth) ou exigir o header
        Authorization: App <api_key>. Usamos a versão com auth por default
        ...
        try:
            print(f"📥 Baixando mídia do Infobip: {media_url}...")
